...
import { Form,Space,Button,DatePicker,Input,ConfigProvider,message} from 'antd';
import {EditOutlined} from '@ant-design/icons';
import locale from 'antd/locale/ru_RU'


function ACFormSecRis({handleSend,name,reset,setReset}) {
    const [analiticFormRising] = Form.useForm();
    const [date, setDate] = useState();

    const handleSubmit = () => {
        analiticFormRising.validateFields().then((objform)=>{
            objform['location'] = name
            objform['recdt'] = new Date(objform.recdt).toISOString()
            handleSend(objform,"rising")
        }).catch((error)=>{
            message.error(error.message)
        })
    }
    const handleReset = () => {
        analiticFormRising.resetFields();
        setDate(null)
    };
    ...
    const handleDateChange = (values) => {
        setDate(values)
        analiticFormRising.setFieldsValue({ recdt: values });
      };
